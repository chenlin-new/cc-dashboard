...
import { Bug, Download, Copy, Play, Check, Upload, ChevronRight, AlertTriangle, Clock, Zap, Terminal, FileText, Server, Wifi, WifiOff, Square, Trash2 } from 'lucide-react'
import { connectArthas, executeArthasLive, generateArthasScript, analyzeArthasLog } from '../api'
import { useLocale } from '../contexts/LocaleContext'
import type { ArthasCommand, ArthasAnalyzeResult } from '../types'
...
const QUICK_COMMANDS: { type: string; label: string; icon: ReactNode; desc: string; method?: boolean; pkg?: boolean; arthasCmd?: (method?: string) => string }[] = [
  ...
  { type: 'logger_debug', label: '开 DEBUG 日志', icon: <FileText className="h-3.5 w-3.5" />, desc: '临时打开包级 DEBUG 日志', pkg: true, arthasCmd: (p) => `logger --name ${p || 'com.fjskec'} --level DEBUG` },
]
...
export default function ArthasPage() {
  ...
  // ── Generator State ──
  const [serviceName, setServiceName] = useState('fjskec-pms-service')
  ...
  const handleConnect = async () => {
    ...
    setConnectError('')
    try {
      const res = await connectArthas(sshHost.trim(), sshUser.trim(), parseInt(sshPort) || 22)
  ...
  const handleLiveExecute = async (cmdType: string, methodOverride?: string) => {
    ...
    let arg = ''
    if (preset.method) {
      arg = methodOverride || liveMethod.trim() || 'com.fjskec.pms.service.xxx.methodName'
    } else if (preset.pkg) {
      arg = 'com.fjskec.pms.mapper'
    }
    ...
    const ctrl = executeArthasLive(
      sshHost.trim(), sshUser.trim(), selectedPid, arthasCmd, parseInt(sshPort) || 22,
      (line) => setLiveOutput(prev => [...prev, line]),
      (code) => {
        setLiveOutput(prev => [...prev, `\n✅ Done (exit code: ${code})`])
        setLiveRunning(false)
        setLiveAbortCtrl(null)
      },
      (err) => {
        setLiveOutput(prev => [...prev, `\n❌ Error: ${err}`])
        setLiveRunning(false)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (const type of selectedCmds) {
        ...
        else if (preset.method && !genMethodInput.trim()) cmd.method = 'com.fjskec.pms.service.xxx.methodName'
        if (preset.pkg) cmd.pkg = 'com.fjskec.pms.mapper'
        commands.push(cmd)
      }
      const methods = genMethodInput.trim() ? genMethodInput.trim().split('\n').filter(Boolean).map(m => m.trim()) : []
      const res = await generateArthasScript({ serviceName, commands, methods, duration: duration ? parseInt(duration) : undefined })
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = filename || 'arthas-debug.sh'
    a.click()
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzing(true)
    try {
      const res = await analyzeArthasLog(logInput)
      setResult(res)
    } catch (e: any) {
      setResult({ sections: [], errors: [String(e.message)], warnings: [], traces: [], threads: [], summary: 'Analysis failed' })
    }
    setAnalyzing(false)
  }
  ...
  const readFile = (file: File) => {
    ...
    reader.onload = () => setLogInput(String(reader.result || ''))
  ...
  const sectionLabel = (type: string) => t(`arthas.section${type.charAt(0).toUpperCase() + type.slice(1)}` as any)
    ...
    <div className="space-y-6">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-all ${
                tab === tabKey ? 'bg-slate-700/70 text-slate-100 shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {icons[tabKey]}
              {labels[tabKey]}
      ...
      {tab === 'direct' && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Left Panel: SSH Config + Commands */}
          <div className="xl:col-span-1 space-y-5">
                ...
                <div className="flex gap-2">
                  {!connected ? (
                    <button
                      ...
                      disabled={connecting || !sshHost.trim() || !sshUser.trim()}
                    ...
                    >
                      {connecting ? <><span className="h-3.5 w-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('arthas.connecting')}</> : <><Wifi className="h-3.5 w-3.5" />{t('arthas.connect')}</>}
                    </button>
                  ) : (
                    <button
            ...
            {connected && (
              <div className="rounded-xl border border-slate-800/50 bg-slate-900/40 p-4">
                ...
                {processes.length === 0 ? (
                  <p className="text-xs text-slate-500">No Java processes found</p>
                ) : (
                  <div className="space-y-1 max-h-[260px] overflow-y-auto pr-1">
                    {processes.map(p => (
                      <button
                        ...
                        className={`flex items-center gap-2.5 w-full rounded-lg p-2 text-left text-xs transition-all font-mono ${
                          selectedPid === p.pid
                            ? 'bg-[var(--cc-accent)]/10 border border-[var(--cc-accent)]/30 text-slate-100'
                            : 'bg-slate-800/30 border border-transparent text-slate-400 hover:border-slate-700/40'
                        }`}
                      >
                        <span className={`text-[10px] tabular-nums ${selectedPid === p.pid ? 'text-[var(--cc-accent)]' : 'text-slate-500'}`}>
                          {p.pid}
                        </span>
              ...
              <div className="rounded-xl border border-slate-800/50 bg-slate-900/40 p-4">
                ...
                <div className="mb-3">
                  <input
                    ...
                    placeholder="com.fjskec.pms.service.OrderService.createOrder"
                ...
                <div className="space-y-1 max-h-[280px] overflow-y-auto pr-1">
                  {QUICK_COMMANDS.map(cmd => (
                    <button
                      key={cmd.type}
                      onClick={() => handleLiveExecute(cmd.type)}
            ...
            {!connected && (
              <div className="rounded-xl border border-dashed border-slate-700/50 bg-slate-900/20 p-5">
                ...
                <p className="text-xs text-slate-600 mt-1">前提: 服务器上已安装 Java (jps 命令可用)</p>
              </div>
            )}
          </div>
            ...
            <div className="rounded-xl border border-slate-800/50 bg-slate-950/80 overflow-hidden h-full flex flex-col min-h-[600px]">
              <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800/50 bg-slate-900/40 shrink-0">
                ...
                <div className="flex gap-1">
                  {liveRunning && (
                    <button onClick={handleLiveStop} className="flex items-center gap-1 rounded px-2 py-1 text-[10px] text-red-400 hover:bg-red-500/10 transition-all">
                      <Square className="h-3 w-3" />{t('arthas.stop')}
                    </button>
                  )}
                  {liveOutput.length > 0 && (
                    <button onClick={clearOutput} className="flex items-center gap-1 rounded px-2 py-1 text-[10px] text-slate-500 hover:text-slate-300 transition-all">
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center space-y-3">
                      ...
                      <p className="text-sm text-slate-600">
                        {connected ? '选择一个 Java 进程，然后点击 Quick Command 执行' : '输入 SSH 信息并点击连接开始'}
                      </p>
                    </div>
                  </div>
                ) : (
      ...
      {tab === 'generator' && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-1 space-y-5">
            <div className="rounded-xl border border-slate-800/50 bg-slate-900/40 p-4">
              <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wider">{t('arthas.serviceName')}</label>
              <input list="pms-services" value={serviceName} onChange={e => setServiceName(e.target.value)} placeholder={t('arthas.serviceNameHint')}
              ...
              <datalist id="pms-services">{PMS_SERVICES.map(s => <option key={s} value={s} />)}</datalist>
            </div>
              ...
              <div className="space-y-1.5 max-h-[320px] overflow-y-auto pr-1">
                {QUICK_COMMANDS.map(cmd => (
                  <button key={cmd.type} onClick={() => toggleCmd(cmd.type)}
                    className={`flex items-start gap-2.5 w-full rounded-lg p-2.5 text-left transition-all ${
                      selectedCmds.has(cmd.type) ? 'bg-[var(--cc-accent)]/10 border border-[var(--cc-accent)]/30' : 'bg-slate-800/30 border border-transparent hover:border-slate-700/40'
                    }`}>
                    ...
                    <span className={`shrink-0 mt-0.5 rounded-full h-4 w-4 flex items-center justify-center text-[10px] ${
                      selectedCmds.has(cmd.type) ? 'bg-[var(--cc-accent)]/20 text-[var(--cc-accent)]' : 'bg-slate-700/50 text-slate-600'
                    }`}>{selectedCmds.has(cmd.type) ? '✓' : ''}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <button onClick={handleGenerate} disabled={scriptLoading}
              className="flex items-center justify-center gap-2 w-full rounded-xl py-3 text-sm font-semibold text-white shadow-lg transition-all"
            ...
            {script ? (<>
              ...
              <div className="flex items-center justify-center h-[500px] rounded-xl border border-dashed border-slate-700/50 bg-slate-900/20">
                <div className="text-center space-y-3 p-8">
                  <Terminal className="h-10 w-10 text-slate-600 mx-auto" />
                  <p className="text-sm text-slate-500 max-w-sm">{t('arthas.scriptGen')} — 选择目标和命令后点击生成</p>
                </div>
              </div>
            )}
